...
"""
Shard Emitter
CREDIT: https://www.reddit.com/r/godot/comments/nimkqg/how_to_break_a_2d_sprite_in_a_cool_and_easy_way/
u/twinpixelriot
"""
export(int, 200) var nbr_of_shards = 20 #sets the number of break points
export(float) var threshhold = 1.0 #prevents slim triangles being created at the sprite edges
export(float) var min_impulse = 20.0 #impuls of the shards upon breaking
...
const SHARD = preload("res://Libs/Shard.tscn")
...
func sprite():
	if get_parent() is AnimatedSprite:
		var parent = get_parent()
		var image = parent.frames.get_frame(parent.animation, parent.frame).get_data()
		var imgTexture = ImageTexture.new()
		imgTexture.create_from_image(image, 0)
		$Sprite.texture = imgTexture
		$Sprite.scale = $Sprite.scale * 10
		$Sprite.update()
		var _rect = $Sprite.get_rect()
		var points = []
		#add outer frame points
		points.append(_rect.position)
		points.append(_rect.position + Vector2(_rect.size.x, 0))
...
func animated():
	if get_parent() is AnimatedSprite:
		...
		var _rect = Rect2(parent.position, image.get_size())
		var points = []
		#add outer frame points
		points.append(_rect.position)
		points.append(_rect.position + Vector2(_rect.size.x, 0))
...
func setup_animated_sprite():
	if get_parent() is AnimatedSprite:
		var parent = get_parent()
		parent.stop()
		var texture = parent.frames.get_frame(parent.animation, parent.frame)
		texture.get_data().get_rect()
		var texture_end = Vector2(parent.position.x + texture.get_width(), parent.position.y + texture.get_height())
		var _size = texture.get_size()
		var points = []
		#add outer frame points
		points.append(parent.position)
		points.append(parent.position + Vector2(_size.x, 0))
		points.append(parent.position + Vector2(0, _size.y))
		points.append(texture_end)

		#add random break points
		for i in nbr_of_shards:
			var p = parent.position + Vector2(rand_range(0, _size.x), rand_range(0, _size.y))
			#move outer points onto rectangle edges
			if p.x < parent.position.x + threshhold:
				p.x = parent.position.x
			elif p.x > texture_end.x - threshhold:
				p.x = texture_end.x
			if p.y < parent.position.y + threshhold:
				p.y = parent.position.y
			elif p.y > texture_end.y - threshhold:
				p.y = texture_end.y
			points.append(p)
